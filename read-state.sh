...
function getStateValueAndPublish() {
  ...
  if [ $EXIT_STATUS -eq 0 ] || { [ $EXIT_STATUS -eq 1 ] && [ $rqdValue == "false" ]; } || { [ $EXIT_STATUS -eq 1 ] && [ $jsonParam == ".chargeState.connChargeCable" ]; }; then
    ...
    ret=2
    log_warning "getStateValueAndPublish; failed to parse $jsonParam for vin:$vin return:$ret"
  fi
...
function readChargeState() {
  ...
  getStateValueAndPublish $vin '.chargeState.chrgePortDoorOpen' cover/charge_port_door_open "$TESLACTRLOUT"
