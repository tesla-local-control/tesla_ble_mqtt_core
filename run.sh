...
if [ ! -d /share/tesla_ble_mqtt ]; then
    ...
    log_info "/share/tesla_ble_mqtt already exists, existing keys can be reused"
fi
...
do
  ...
  if [ $PRESENCE_DETECTION_TTL -gt 0 ] ; then
    log_notice "Launch BLE scanning for car presence every $PRESENCE_DETECTION_LOOP_DELAY seconds"
