...
- NEW Features:
   ...
   - Environment variable $OPTIMISTIC_MODE can be set to false. When false, entities will immediately change state after a command is sent from HA (even if the command ultimately fails). If true (default), the entity will wait for state confirmation via polling or a force update button press
